...
public class Executor {
    ...
    public class ConcurrencyAdjuster implements Runnable {
        ...
        private boolean canRefreshConcurrency(ConcurrencyType concurrencyType) {
            ...
            return switch (concurrencyType) {
                case LEADERSHIP_CLUSTER, LEADERSHIP_BROKER ->
                        _executorState.state() == LEADER_MOVEMENT_TASK_IN_PROGRESS;
    ...
    private void sanityCheckOngoingMovement() throws OngoingExecutionException {
        ...
        try {
            ...
            hasOngoingPartitionReassignments = !ongoingPartitionReassignments.isEmpty();
    ...
    public synchronized void shutdown() {
        ...
        try {
            if (!_proposalExecutor.awaitTermination(Long.MAX_VALUE, TimeUnit.MILLISECONDS)) {
                LOG.warn("Timed out while waiting for proposal executor to shutdown.");
                _proposalExecutor.shutdownNow();
            }
        } catch (InterruptedException e) {
            LOG.warn("Interrupted while waiting for proposal executor to shutdown.");
            _proposalExecutor.shutdownNow();
