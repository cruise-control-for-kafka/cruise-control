...
public class ExecutionTaskManager {
    ...
    public synchronized List<ExecutionTask> getInterBrokerReplicaMovementTasks() {
        Map<Integer, Integer> brokersReadyForReplicaMovement = brokersReadyForReplicaMovement(_inProgressInterBrokerReplicaMovementsByBrokerId, ConcurrencyType.INTER_BROKER_REPLICA);
        return _executionTaskPlanner.getInterBrokerReplicaMovementTasks(brokersReadyForReplicaMovement, _inProgressPartitionsForInterBrokerMovement, _executionConcurrencyManager.maxClusterInterBrokerPartitionMovements());
    }
    ...
    public synchronized List<ExecutionTask> getIntraBrokerReplicaMovementTasks() {
        Map<Integer, Integer> brokersReadyForReplicaMovement = brokersReadyForReplicaMovement(_inProgressIntraBrokerReplicaMovementsByBrokerId, ConcurrencyType.INTRA_BROKER_REPLICA);
     ...
     */
    private Map<Integer, Integer> brokersReadyForReplicaMovement(Map<Integer, Integer> inProgressReplicaMovementsByBrokerId, ConcurrencyType concurrencyType) {
        Map<Integer, Integer> readyBrokers = new HashMap<>();
        int unthrottledConcurrency = _executionConcurrencyManager.unthrottledConcurrency(inProgressReplicaMovementsByBrokerId.keySet(), _brokersToSkipConcurrencyCheck);
        inProgressReplicaMovementsByBrokerId.forEach((brokerId, inProgressReplicaMovements) -> {
            int brokerConcurrency = _brokersToSkipConcurrencyCheck.contains(brokerId) ? unthrottledConcurrency : _executionConcurrencyManager.getExecutionBrokerConcurrency(brokerId, concurrencyType);
     ...
     */
    public synchronized void addExecutionProposals(Collection<ExecutionProposal> proposals, Collection<Integer> brokersToSkipConcurrencyCheck, StrategyOptions strategyOptions, ReplicaMovementStrategy replicaMovementStrategy) {
        ...
        for (ExecutionProposal p : proposals) {
            p.replicasToMoveBetweenDisksByBroker().keySet().forEach(broker -> _inProgressIntraBrokerReplicaMovementsByBrokerId.putIfAbsent(broker, 0));
        ...
        for (ExecutionTask task : tasks) {
            ...
            switch (task.type()) {
                case INTER_BROKER_REPLICA_ACTION:
                    ...
                    _inProgressInterBrokerReplicaMovementsByBrokerId.put(oldLeader, _inProgressInterBrokerReplicaMovementsByBrokerId.get(oldLeader) + 1);
                    task.proposal().replicasToAdd().forEach(r -> _inProgressInterBrokerReplicaMovementsByBrokerId.put(r.brokerId(), _inProgressInterBrokerReplicaMovementsByBrokerId.get(r.brokerId()) + 1));
                    break;
                case INTRA_BROKER_REPLICA_ACTION:
                    _inProgressIntraBrokerReplicaMovementsByBrokerId.put(task.brokerId(), _inProgressIntraBrokerReplicaMovementsByBrokerId.get(task.brokerId()) + 1);
                    break;
                default:
                    break;
            }
    ...
    /**
     * @return Number of remaining intra broker partition movements.
     */
     ...
     */
    public synchronized long inExecutionIntraBrokerDataMovementInMB() {
